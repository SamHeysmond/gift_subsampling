...
for phenotype in phenotypes_input:
    ...
    for subsample_num in subsample_list:
        ...
        for copynum in range(1,100):
            ...
            bash_script_output.write('python3 core_files/physics_GWAS_OOP.py -v core_files/subsampled_genotype_${i}_${SLURM_JOB_ID}.vcf -f core_files/subsampled_phenotype_${i}_${SLURM_JOB_ID}.csv -p ${phenotype} -o output_files/${phenotype}_whole_genome_metrics_${i}_${SLURM_JOB_ID}.csv -id ${SLURM_JOB_ID}\n')
            bash_script_output.write(f'# exit gift environment\n')
            bash_script_output.write(f'conda deactivate\n')
            bash_script_output.write('echo "GIFT for: ${SLURM_JOB_ID} finished"\n')
            bash_script_output.write(f'# remove the subsampled vcf file to save on storage (TEMP PAUSED)\n')
